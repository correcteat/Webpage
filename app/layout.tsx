...
export default function RootLayout({children}:{children:React.ReactNode}){return <html lang="el"><body><Header/>{children}<Footer/><script dangerouslySetInnerHTML={{__html:`document.addEventListener('DOMContentLoaded',()=>{document.querySelectorAll('a').forEach(a=>{if(a.textContent?.trim()==='Κλείσε ραντεβού ↗'||a.textContent?.trim()==='Κλείσε ραντεβού'){a.childNodes.forEach(n=>{if(n.nodeType===3&&n.textContent?.includes('Κλείσε ραντεβού'))n.textContent='Κλείσε το ραντεβού σου εδώ ↗';});}});});`}}/></body></html>}
